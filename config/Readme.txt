...
- Place the jmx2insights section directly after the proxy section in newrelic.yml (You can place it anywhere, but I place it after proxy config).
  ...
  # mbean_n: (string) the mbean name and names of the attributes required (a unique polling interval can be specified)
...
- The service is aware of changes to the newrelic.yml file, so changes will manifest a few minutes after newrelic.yml has been updated, this will depend on the frequency you have set for polling.
	...
	- memory_events: Enables a 1 second polling interval for JVM Memory related MBeans.
	- beta_features: Enables select beta features TBD.
	- mbean_N: There can be multiple "mbean" options defined in the jmx2insights section, this forms the list of defined MBean polled in strict or open mode.
		...
		- if you want to define a unique polling interval for an mbean append a number to the mbean definition after the final square bracket "]" (e.g. com.mypackage:type=ImportantData [Attribute0,Attribute1,Attribute2]5) - this works in strict and open configurations
...
2.7
- Support for the tabular or composite data fields with new mbean definition syntax: 
-- mbean_50: org.apache.jackrabbit.oak:type=RepositoryStats,name=Oak Repository Statistics [ObservationQueueMaxLength(per minute|per hour),SessionWriteCount(per second|per minute|per hour),SessionReadAverage(per second),SessionReadDuration(per second),SessionReadCount(per second)]
v2.6.3
